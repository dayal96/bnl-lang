...
public class IsEmpty extends AOperator {

  @Override
  public Expression evaluate(List<Expression> operands, Environment environment) throws Exception {
    if (operands.size() != 1) {
      throw new Exception("empty? : expected 1 argument, found " + operands.size());
    }

    try {
      Empty.EMPTY.getType().join(operands.get(0).evaluate(environment).getType());
    } catch (Exception e) {
